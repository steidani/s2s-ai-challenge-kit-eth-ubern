...
from helper_ml_data import load_data, get_basis, rm_annualcycle, DataGenerator, single_prediction, skill_by_year_single
...
# ## Hindcast
hind_2000_2019 = load_data(data = 'hind_2000-2019', aggregation = 'biweekly', path = path_data,var_list = ['tp','t2m','sm20','sst'])#,'lsm', 'msl', var_list = ['tp','t2m'])#path_data)
if 'sm20' in hind_2000_2019.keys():
    hind_2000_2019 = hind_2000_2019.isel(depth_below_and_layer = 0 ).reset_coords('depth_below_and_layer', drop=True)
elif 'msl' in hind_2000_2019.keys():
    hind_2000_2019 = hind_2000_2019.isel(meanSea = 0).reset_coords('meanSea', drop = True)
...
input_lat = slice(90,-90)#slice(90,-20)
input_lon = slice(0, 360) #slice(0,90)  slice(300,360)#negative,positive will not work
...
acc_per_fold = [] ##convert to dataframe that also contains standardization rmcycle 
loss_per_fold = []
skill = []
...
for standardization in [True, False]:
    for rmcycle in [True, False]:
        ...
        for fold_no in range(0,2):#range(0,10):
            ...
            if rmcycle == True:
                var_list = [i for i in fct_train.keys() if i not in ['lsm']]
            ...
            inp_imgs = Input(shape=(121,240,4,)) #fcts4
            ...
            skill.append(skill_by_year_single(preds_single, 
...
results = pd.DataFrame(np.column_stack([param_standardization, param_rmcycle, acc_per_fold,loss_per_fold,skill]), 
                               columns=['standardization', 'rm annual cycle', 'accuracy','loss','skill'])
          
# In[30]: 
    
#results.to_csv('results_t2m_34_tp-t2m-sm20-sst.csv', sep =';', index = False)   
