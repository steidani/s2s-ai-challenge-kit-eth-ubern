...
def load():
    ...
    hind_2000_2019_raw = load_data(data='hind_2000-2019', aggregation='biweekly', path=path_data, var_list=[
        't2m'])  # ['tp','t2m','sm20','sst'])#,'lsm', 'msl', var_list = ['tp','t2m'])#path_data)
...
def main():
    ...
    for i in range(0, len(input_lats)):
        ...
        for fold_no in range(0, 2):  # range(0,10):
            ...
            inp_imgs = Input(
                shape=(dg_train[0][0][0].shape[1], dg_train[0][0][0].shape[2], dg_train[0][0][0].shape[3],))
            # fcts4121,240,
    ...
    # setup_params, #fold_no, input_lat, output_lat, standardization,
    # rmcycle, rmterciles, bs, ep, basis_rad, fct_train.keys()
