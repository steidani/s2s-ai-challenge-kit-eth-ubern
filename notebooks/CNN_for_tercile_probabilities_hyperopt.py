...
# 
# This notebook create a Machine Learning `ML_model` to predict weeks 3-4 & 5-6 based on `S2S` weeks 3-4 & 5-6 forecasts
# and is compared to `CPC` observations for the [`s2s-ai-challenge`](https://s2s-ai-challenge.github.io/).

# https://s2s-ai-challenge.github.io/
# 
# We deal with two fundamentally different variables here: 
# - Total precipitation is precipitation flux pr accumulated over lead_time until valid_time and therefore describes
# a point observation.
# - 2m temperature is averaged over lead_time(valid_time) and therefore describes an average observation. 
# 
# The submission file data model unifies both approaches and assigns 14 days for week 3-4 and 28 days for week 5-6
# marking the first day of the biweekly aggregate.
...
# ### Select region
# 
# * use the whole globe as input since global teleconnections are a major source of predictability,
# at least over Europe.
# * create predictions for a much smaller domain, since otherwise the basis functions
# (and the associated multiplication) uses too much memory.
# 
...
def evaluate(params):
    ...
    for i in range(0, 1):  # todo range(0, len(input_lats)):
        # print(i)
        # input_lon = input_lons[i] todo
        # input_lat = input_lats[i]

        # output_lon = output_lons[i]
        # output_lat = output_lats[i]

        input_lon = input_lons[3]
        input_lat = input_lats[3]
        output_lon = output_lons[1]
        output_lat = output_lats[1]

        rad = [4, 8, 16, 32]  # [4,6,7,10]
        # basis_rad = rad[i] todo
        basis_rad = rad[1]
        ...
        for fold_no in range(0, 1):  # range(0,10):
            ...
            # train
            fct_train = hind_2000_2019.isel(forecast_time=train_indices)  # [v]
            ...
            # don't divide by local standard deviation!

            # fct_train.sel(lead_time = lead_input).isel(forecast_time = 0)['msl'].plot()#'t2m'
            # t2m -10 bis 10, tp: -150 bis 150, sm20: -200 bis 200
            # --> maybe some rescaling and cutting of the outliers would help,
            # sst: -3 bis 3, slm: 0 bis 1, msl: -1500 bis 1500.
            ...
            inp_imgs = Input(shape=(dg_train[0][0][0].shape[1],
                                    dg_train[0][0][0].shape[2],
                                    dg_train[0][0][0].shape[3],))
            # fcts4121,240,
            inp_basis = Input(shape=(n_xy, n_basis))  # basis
            inp_cl = Input(shape=(n_xy, n_bins,))  # climatology

            c = Conv2D(4, (3, 3), activation=activation_function)(inp_imgs)
            c = MaxPooling2D((2, 2))(c)
            # c = Conv2D(8, (3,3), activation=activation_function)(c)
            # c = MaxPooling2D((2,2))(c)
            x = Flatten()(c)
            for h in hidden_nodes:
                x = Dropout(dropout_rate)(x)
                x = Dense(h, activation=activation_function)(x)
            x = Dense(n_bins*n_basis, activation=activation_function)(x)
            ...
            cnn.compile(loss=keras.losses.CategoricalCrossentropy(label_smoothing=smoothing), metrics=['accuracy'],
...
if __name__ == '__main__':
    space = [
        hp.choice('activation_function', ['relu', 'elu']),
        hp.choice('learning_rate', [5e-5, 1e-4, 2e-4, 3e-4]),
        hp.choice('cnn_layers', [[]]),  # todo not implemented yet
        hp.choice('batch_size', [16, 32, 64, 128]),
        hp.choice('dropout_rate', [0.3, 0.4, 0.5, 0.6]),
        hp.choice('epochs', [5, 10, 15, 20]),
        hp.choice('smoothing', [0, 0.2, 0.4, 0.6])
    ]
