""" 
investigate 3D hyperparameter space: output domain size, radius of basis function, label smoothing.
"""
...
output_lats = [slice(63,52),slice(69,46), slice(81,34)]#8, 16, 32
output_lons = [slice(67,78), slice(61,85), slice(49,97)]

for i  in  range(0, len(output_lats)):
    print(i)
    input_lon = input_lons[0]
    input_lat = input_lats[0]
...
setup = pd.DataFrame(setup_params, columns = ['fold_no','output_lats', 'output_lons', 
                                              'input_lats', 'input_lons', 'radius_basis_func', 'label_smoothing', 
...
#results.to_csv('E3_results_t2m_34_t2m.csv', sep =';', index = False)