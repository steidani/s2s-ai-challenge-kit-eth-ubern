...
# =============================================================================
    ...
    #fold_no = 0
    for fold_no in range(0,10):#range(0,10):
...
# =============================================================================
#         def rm_coords_to_var(da):
#             da = da.assign_coords( category_edge = ['lower_{}'.format(da.name),'upper_{}'.format(da.name)]).to_dataset(dim ='category_edge')
#             return da
# =============================================================================
        
        if rmterciles == True: 
            #tercile_edges_v = load_data(data = 'obs_tercile_edges_2000-2019', aggregation = 'biweekly', path = path_data).isel(lead_time = lead_output)
            ...
            for var in list(deb.keys()):
                #print(var)
                debs.append(deb[var].assign_coords( category_edge = ['lower_{}'.format(var),'upper_{}'.format(var)]).to_dataset(dim ='category_edge'))
            deb_valid = xr.merge(debs)#.map(rm_coords_to_var)
...
# =============================================================================
#             deb = rm_tercile_edges(fct_train[vars_], tercile_edges[vars_]).assign_coords( category_edge = ['lower_{}'.format(vars_),'upper_{}'.format(vars_)]).to_dataset(dim ='category_edge')
...
# =============================================================================
        ...
        preds_single = single_prediction(cnn, 
                                     [fct_valid.fillna(0.).to_array().transpose('forecast_time', ...,'variable').values, #.sel(lead_time = lead_input)
        ...
        preds_masked = preds_single.where(mask[v].notnull())#.sel(lead_time = lead_output)
